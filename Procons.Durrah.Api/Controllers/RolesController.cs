namespace Procons.Durrah.Controllers
{
    ...
    public class RolesController : BaseApiController
    {
        ...
        [Authorize]
        [Route("GetCurrentRole", Name = "GetCurrentRole")]
        public async Task<IHttpActionResult> GetRole()
        {
            ...
            var modelfactories = new List<RoleReturnModel>();
            if (roles.Count() > 0)
            {
                foreach (var x in roles)
                {
                    var role = await this.AppRoleManager.FindByNameAsync(x);
                    modelfactories.Add(TheModelFactory.Create(role));
                }
                return Ok(modelfactories);
            }
            else
                return NotFound();
        }
